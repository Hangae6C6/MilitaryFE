...
const addUserDataDB = (userId, startDate, endDate, armyCategory, rank) => {
  ...
  return async function (dispatch) {
    ...
    try {
      await axios({
        method: "post",
        url: `https://soldierchallengers.com/api/userData?userId=${userId}`,
...
const addTestResultDB = (userId, result) => {
  return async function () {
    try {
      await axios({
        method: "post",
        url: "https://soldierchallengers.com/api/userTest",
        data: {
          userId,
          testResult: result,
        },
...
const getUserProfileDB = (userId) => {
  return async function (dispatch) {
    try {
      await axios({
        method: "get",
        url: `https://soldierchallengers.com/api/myPage/userProfile?userId=${userId}`,
        headers: {
          Authorization: `Bearer ${getCookie("token")}`,
        },
      }).then((response) => {
